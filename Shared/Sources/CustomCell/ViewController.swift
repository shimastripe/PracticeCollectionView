...
extension CustomCellViewController {
    ...
    /// 差分検出を自前で行う必要がある
    func reconfigureSnapshot(ids: [UUID]) -> NSDiffableDataSourceSnapshot<Section, UUID> {
    ...
    func configureNavBarButton() {
        reloadButton = .init(primaryAction: .init(title: "ID 変更(Reload)", handler: { [weak self] _ in
        ...
        updateButton = .init(primaryAction: .init(title: "ID 維持(Update)", handler: { [weak self] _ in
            guard let self else { return }
            var newItems = list
            newItems[1] = Item(id: list[1].id, bgColor: list[3].bgColor)
            newItems[3] = Item(id: list[3].id, bgColor: list[1].bgColor)
            list = newItems
            let snapshot = reconfigureSnapshot(ids: [newItems[1].id, newItems[3].id])
            // MEMO: animatingDifferences: false にすると、SwiftUIと同じ動き
            dataSource.apply(snapshot)
        }))

        navigationItem.rightBarButtonItems = [.init(customView: reloadButton), .init(customView: updateButton)]
    }
}
