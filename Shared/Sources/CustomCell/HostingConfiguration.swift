...
struct HostingCell: View {

    let icons = ["star", "star.fill", "star.slash"]

    @State var icon = "star"

    let item: HostingViewController.Item

    var body: some View {
        HStack {
            Text(item.id.uuidString).layoutPriority(1)
            Spacer()
            Button(action: {
...
public final class HostingViewController: UIViewController {
